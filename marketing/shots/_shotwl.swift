...
final class Nav: NSObject, WKNavigationDelegate {
  func webView(_ w: WKWebView, didFinish n: WKNavigation!) {
    DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
      w.evaluateJavaScript("try{localStorage.clear();}catch(e){}; var el=document.getElementById('waitlist'); if(el){el.scrollIntoView();} el? 'found':'MISSING'") { r, _ in
        print("waitlist:", r ?? "?")
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
          ...
          w.takeSnapshot(with: sc) { image, _ in
            if let img = image, let t = img.tiffRepresentation, let rep = NSBitmapImageRep(data: t),
              ...
              try? png.write(to: URL(fileURLWithPath: "/tmp/qc-waitlist.png")); print("saved")
            }
            exit(0)
          }
        }
