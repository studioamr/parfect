...
let steps: [(String, String, Double)] = [
  ("09-analisis", "document.querySelector(\"[data-view='trainer']\").click();", 1.8),
  ("10-min30", "var b=[...document.querySelectorAll('button')].find(x=>/^\\s*Entreno\\s*$/.test(x.textContent||'')); if(b)b.click(); setTimeout(function(){var c=[...document.querySelectorAll('button,[role=button],div')].filter(x=>/30\\s*min/.test(x.textContent||'') && (x.textContent||'').length<25); if(c.length)c[c.length-1].click();},600);", 2.4),
]
