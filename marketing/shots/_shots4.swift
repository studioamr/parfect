...
let steps: [(String, String, Double)] = [
  ("12-nueva", "var p=[...document.querySelectorAll('button,[role=button],a')].find(x=>(x.textContent||'').trim()==='P'); if(p)p.click();", 2.0),
  ("13-campo", "var c=[...document.querySelectorAll('button,[role=button],div')].filter(x=>/Tres Mar/.test(x.textContent||'') && (x.textContent||'').length<80); if(c.length)c[c.length-1].click();", 2.0),
  ("14-scorecard", "var e=[...document.querySelectorAll('button')].find(x=>/Empezar|Iniciar|Comenzar|Jugar/.test(x.textContent||'') && (x.textContent||'').length<30); if(e)e.click();", 2.0),
]
